...
fun ByteArray.convertBits(fromBits: Int, toBits: Int, pad: Boolean = true): ByteArray {
    var acc = 0
    var bits = 0
    ...
    forEach { byte ->
        val valueAsInt: Int = (byte.toInt() and 0xff)
        val rBit = (valueAsInt shr fromBits)
        if (valueAsInt < 0 || rBit != 0) {
            return emptyList<Byte>().toByteArray()
        }
